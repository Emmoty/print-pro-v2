...
async function getAccessToken() {
  ...
  if (!consumerKey || !consumerSecret || consumerKey.includes('YOUR_')) {
    // If keys not yet configured in .env, log diagnostic warning and provide simulated fallback
    return 'sandbox_daraja_access_token_placeholder';
  }
  ...
  throw new Error('Could not obtain M-Pesa Daraja access token.');
}
...
async function initiateSTKPush({ phone, amount, jobId, callbackUrl, accountReference }) {
  ...
  const consumerKey = process.env.MPESA_CONSUMER_KEY;
  if (!consumerKey || consumerKey.includes('YOUR_')) {
    // Graceful test simulation if user has not yet entered live credentials in .env
    const mockCheckoutId = 'ws_CO_' + timestamp + '_' + Math.floor(1000 + Math.random() * 9000);
    return {
...
async function querySTKStatus(checkoutRequestId) {
  ...
  const consumerKey = process.env.MPESA_CONSUMER_KEY;
  if (!consumerKey || consumerKey.includes('YOUR_')) {
    const mockReceipt = 'UHUF' + Math.random().toString(36).substring(2, 8).toUpperCase();
    return {
      ResponseCode: '0',
      ResultCode: '0',
  ...
  try {
    ...
    if (err.body && (err.body.errorMessage?.includes('being processed') || err.body.errorCode === '500.001.1001')) {
      return { isPending: true, message: 'Transaction still pending on phone' };
    }
    return { isPending: true, error: err.message };
  }
}
