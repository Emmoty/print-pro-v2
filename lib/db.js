...
async function runSchemaMigrations() {
  ...
  try {
    ...
    // Explicit safety check for transactions table
    await pgPool.query(`
  ...
  if (pgPool && isPgConnected) {
    pgPool.query(
      `INSERT INTO transactions (id, job_id, mpesa_receipt_number, amount, phone, recorded_at, status)
       ...
       ON CONFLICT (id) DO NOTHING`,
      [txRecord.id, txRecord.jobId, txRecord.mpesaReceiptNumber, txRecord.amount, txRecord.phone, txRecord.recordedAt, txRecord.status]
